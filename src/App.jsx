...
export default function KyudoStringCustomizer() {
  ...
  // 素材が変更されたとき、現在選択されている色が利用不可ならデフォルト（最初の利用可能色）に変更
  useEffect(() => {
    if (material.availableColors && !material.availableColors.includes(bodyColor.id)) {
      const firstAvailableColor = COLORS.find(c => material.availableColors.includes(c.id));
        ...
        <div className={`
          ...
          <div className="w-full h-full flex items-center justify-center bg-stone-50/50 relative">
            ...
            >
              ...
              <g transform={`${isMobile ? "translate(-15, 100) rotate(-90)" : "translate(25, 20) scale(1.5)"} ${hinowaRot ? (isMobile ? 'rotate(180, 50, 170)' : 'rotate(180, 50, 300)') : ''}`}>
                <StringLoopDetail loopColor={hinowaColor.hex} bodyColor={bodyColor.hex} isMobile={isMobile} />
              </g>
              <g transform={`${isMobile ? "translate(-15, 230) rotate(-90)" : "translate(225, 20) scale(1.5)"} ${tsukinowaRot ? (isMobile ? 'rotate(180, 50, 170)' : 'rotate(180, 50, 300)') : ''}`}>
